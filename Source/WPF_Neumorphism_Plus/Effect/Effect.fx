...
float4 outerShadowCalculator(float2 uv :TEXCOORD, float4 ShadowColor, float OffsetDirection) :COLOR
{
        ...
        UnusedPaddingLeft < uv.x && uv.x < UnusedPaddingRight && UnusedPaddingTop < uv.y && uv.y < UnusedPaddingBottom)
    {
        ...
        // 2
        if (UnusedPaddingTop < uv.y && uv.y < UnusedPaddingTop + BlurEffectiveLengthY
            &&
            UnusedPaddingLeft + BlurEffectiveLengthX < uv.x && uv.x < UnusedPaddingRight - BlurEffectiveLengthX)
        ...
        // 7
        if (UnusedPaddingBottom - BlurEffectiveLengthY < uv.y && uv.y < UnusedPaddingBottom
            &&
            UnusedPaddingLeft + BlurEffectiveLengthX < uv.x && uv.x < UnusedPaddingRight - BlurEffectiveLengthX)
